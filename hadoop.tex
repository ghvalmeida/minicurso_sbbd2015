...
\citep{ghemawat2003google}, que era um sistema de arquivos distribuído usado em sua máquina de busca. 
...
Em 2004, a equipe do Google publicou um novo artigo detalhando como era possível criar um índice de 
toda a Internet usando o mecanismo de processamento paralelo denominado MapReduce \citep{dean2008mapreduce}. 
...
frameworks foram construídos usando a sua infraestrutura, como será visto a seguir. 
...
muito grandes\footnote{Atualmente há instâncias do HDFS armazenando PetaBytes de dados.} executando em computadores padrão de baixo custo. 
...
A arquitetura de um \textit{cluster} HDFS divide-se em \textit{NameNode} e \textit{DataNode}. O primeiro armazena um índice de arquivos e de seus blocos e o segundo armazena os dados (blocos). Um cliente que queira ler arquivos no HDFS, primeiro consulta o \textit{NameNode}, que então diz de quais nós do cluster os blocos serão lidos, garantindo um balanceamento de carga na leitura. Arquivos criados no HDFS só podem ser modificados anexando conteúdo no final. Não é possível modificar blocos já escritos. A falha do \textit{DataNode} implica na indisponibilidade de todo o HDFS e, por esse motivo, é importante mantê-lo resiliente a falha com mecanismos de redundância. Uma visão geral dessa arquitetura pode ser vista na figura \ref{fig:hdfs_arch}.
...
A Figura \ref{fig:mapreduce} apresenta um desenho esquemático de um \textit{job} MapReduce, que é, 
...
Suri e Sergei \citep{suri2011counting}, envolve o encadeamento de 3 \textit{jobs} MapReduce. O primeiro 
...
Dado o grafo não direcionado de entrada descrito na Tabela \ref{grafoExemplo}, onde os nomes identificam 
...
Após a etapa de \textit{Map} do primeiro \textit{job}, são gerados um conjunto de pares
...
um desenvolvimento de software em linguagens de programação. Assim surgiu rapidamente a necessidade de 
especificação de programas MapReduce em linguagens declarativas de alto nível.

O Hive \citep{thusoo2009hive} é uma das iniciativas neste sentido, no qual a especificação de 
\textit{jobs} MapReduce é feita em uma linguagem similar ao SQL: o HiveQL. O Hive foi inicialmente desenvolvido 
...
Como um exemplo, imagine que há um arquivo contendo os dados do Twitter no HDFS contendo 
...
De forma similar ao Hive, o Yahoo! criou o Pig \citep{olston2008pig} em 2006 e o moveu como um projeto 
...
em três tabelas não deveria ser necessária, mas na versão do Pig (antiga) em que foi criado e executado
...
grafos, fluxo contínuo de dados e SQL. Algumas dessas são vistas a seguir.
...
suportada é o Scala. Atualmente ele conta com algoritmos de PageRank \citep{brin2012reprint}, identificação 
de componentes conectados e contagem de triângulos, que é demonstrada com trecho de código abaixo. 
